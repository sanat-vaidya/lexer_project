...
------------------------------------------------------------------------------------
	struct set_of_states table2[5][3] = {
		//   epsilon        '0'          '1'
		{{1,{1}},      {1,{2}},     {0,{}}},     // state 0: e->{1}, 0->{2}, 1->{}
		{{1,{3}},      {0,{}},      {1,{4}}},    // state 1: e->{3}, 0->{},  1->{4}
		{{0,{}},       {1,{2}},     {0,{}}},     // state 2: e->{},  0->{2}, 1->{}
		{{0,{}},       {0,{}},      {1,{4}}},    // state 3: e->{},  0->{},  1->{4}
		{{0,{}},       {0,{}},      {0,{}}}      // state 4: e->{},  0->{},  1->{}
	};

	int enfa_states2[] = {0, 1, 2, 3, 4};
...
/* Basic idea is to write a regular expression, convert it to an E-NFA -> DFA -> minimize DFA (done!) -> run the algorithm (done!)
 *
...
int main(){
	...
	struct DFA minimized = minimize_dfa(dfa2);

	run_dfa(minimized, str_ptr);
 ...
 * when adding random names for states, giving seg fault -> somewhere indexing is not being done right
 ...
 - increasing order of breaking loops in convert_to_dfa to avoid redundant checking
 - a better mapping from state name to index
*/
